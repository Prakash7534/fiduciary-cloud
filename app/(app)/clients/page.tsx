      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white border border-[#CBD9DC] rounded-xl p-5">
          ...
          <table className="w-full text-sm mt-3">
            ...
            <tbody>
              <tr className="text-[#0F3A46] font-medium">
        ...
        <div className="space-y-2">
          {badFlags.map((f, i) => (
            <div key={i} className="flex gap-3 bg-[#F8E7E4] rounded-lg p-3 text-sm">
              ...
              <div>
                <b className="text-[#0F3A46]">{f.name}{f.val ? ` — ${f.val}` : ""}</b>
                <div className="text-xs text-[#5A3A37] mt-0.5">{f.why}</div>
              </div>
            </div>
          ))}
          {okFlags.map((f, i) => (
            <div key={i} className="flex gap-3 bg-[#E4F1EA] rounded-lg p-3 text-sm">
              ...
              <b className="text-[#0F3A46]">{f.name}</b>
            </div>
          ))}
        </div>
      </div>
      ...
      {a.goals.length > 0 && (
        <div className="bg-white border border-[#CBD9DC] rounded-xl p-5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {a.goals.map((g) => (
                <tr key={g.goal_id} className="border-b border-[#E7EFEF] text-[#0F3A46]">
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <tr className="border-b border-[#E7EFEF]">
      ...
      <td className={`py-1.5 text-right text-[#0F3A46] ${bold ? "font-bold" : ""}`}>{value}</td>
    </tr>
  );
}
