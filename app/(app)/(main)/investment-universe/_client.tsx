...
export default function UniverseClient({ initialData }: { initialData: Instrument[] }) {
  ...
  const handleRefreshPrices = async () => {
    ...
    const results: { name: string | null; price?: number; error?: string; source?: string }[] = [];
    let updated = 0;
    ...
    setRefreshResult({ updated, failed: results.filter(r => r.error).length, total: data.length, results });
    setRefreshing(false);
  };
