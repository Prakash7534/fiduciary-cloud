...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (status === "executed") {
    ...
    // Real money just landed in the live portfolio — worth its own
    // History/Trend Analysis point, not just whatever the last questionnaire said.
    await createSnapshot(supabase, id, `Recommendation executed: ${rec.scrip_name} (${rec.doc_id})`);
  }
